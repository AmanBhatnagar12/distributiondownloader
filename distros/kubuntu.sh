...
then
       ...
       do
          if [ $tydosycb = "32bits" ];
                 then
                      urldedescarga="http://cdimage.ubuntu.com/kubuntu/releases/saucy/release/kubuntu-13.10-desktop-i386.iso"
          elif [ $tydosycb = "64bits" ]
                  then
                      urldedescarga="http://cdimage.ubuntu.com/kubuntu/releases/saucy/release/kubuntu-13.10-desktop-amd64.iso"
          fi
       done
fi
...
function DESCMAIN {
archinfo=`dirname $0`/kubuntu.html
zenity --text-info \
       --title="Descargar Kubuntu" \
       --filename=$archinfo \
       --html \
       --width=550 \
       --height=550 \
