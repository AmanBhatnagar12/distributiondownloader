...
then
       ...
       do
          if [ $tydosycb = "32bits" ];
                 then
                      urldedescarga="http://mirror.fsf.org/trisquel-images/trisquel_6.0_i686.iso"
          elif [ $tydosycb = "64bits" ]
                  then
                      urldedescarga="http://mirror.fsf.org/trisquel-images/trisquel_6.0_amd64.iso"
          fi
       done
fi
...
function DESCMAIN {
archinfo=`dirname $0`/trisquel.html
zenity --text-info \
       --title="Descargar Trisquel" \
       --filename=$archinfo \
       --html \
       --width=550 \
       --height=550 \
